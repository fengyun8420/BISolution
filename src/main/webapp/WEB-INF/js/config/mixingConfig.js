var Ticket = function(){
	...
	*/
	...
	{
		$.post("/sva/mixing/api/getData",function(data){
			if (!data.error) {
		...
		bindClickEvent:function()
		{
    		...
    		$("#floorId").on("change", function(){
         	...
         	$("input[data-type='del']").live("click",function(e){
                ...
                if(confirm("delete data?")){
                	$.post("/sva/mixing/api/deleteData",param,function(data){
                		if (data.reslut!=0) {
                			refreshTable();
						}else
							{
            ...
            $("#Confirm").on("click",function(e){
            	...
            	var floorCode = $("#floorCode").val();
          	  	...
          	  	if (storeCode=="") {
              	  	setTip("notPass",inotnull,"storeCodeId");
              	  	check = false;
              	  	}else
              	  		{
                  	  	setTip("pass",ipass,"storeCodeId");
              	  		}
          	  	if (floorCode=="") {
              	  	setTip("notPass",inotnull,"floorCodeId");
              	  	check = false;
              	  	}else
              	  		{
                  	  	setTip("pass",ipass,"floorCodeId");
              	  		}    
          	  	if (mapId==null||mapId=="") {
...
function editData(e)
{
	...
	$("#storeCodeId").val(data.storeCode);
	$("#floorCodeId").val(data.floorCode);
	$("#idid").val(data.id);
