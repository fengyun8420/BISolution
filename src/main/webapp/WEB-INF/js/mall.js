...
//	var times = $.cookie("times");
	$.post("/sva/heatmap/api/getData?t="+Math.random(),{mapId : mapId}, function(data) {
		if (!data.error) {
...
//		}
	...
	var changeFloor = function(placeId, mapId, callback){
		$.post("/sva/heatmap/api/getFloorsByMarket", {
			placeId : placeId
		}, function(data) {
			if (!data.error) {
				var floors = data.data;
				updateFloorList("floorSel", floors, mapId, function() {
	...
	return {
		...
		bindClickEvent : function() {
			...
			$('#confirm').click(function(e) {
				var placeId = $("#marketSel").val();
				var mapId = $("select[name='floorSelName']").find("option:selected").val();
